...
public abstract class MyQueryAction {
    ...
    public MyQueryAction(RestHighLevelClient client, Query query) {
        this.client = client;
        this.query = query;
    }

    protected void updateRequestWithCollapse(Select select, SearchSourceBuilder request) throws SqlParseException {
        ...
        for (Hint hint : select.getHints()) {
            if (hint.getType() == HintType.COLLAPSE && hint.getParams() != null && 0 < hint.getParams().length) {
                try (JsonXContentParser parser = new JsonXContentParser(NamedXContentRegistry.EMPTY, LoggingDeprecationHandler.INSTANCE, jsonFactory.createParser(hint.getParams()[0].toString()))) {
                    request.collapse(CollapseBuilder.fromXContent(parser));
                } catch (IOException e) {
    ...
    protected void updateRequestWithPostFilter(Select select, SearchSourceBuilder request) {
        for (Hint hint : select.getHints()) {
            if (hint.getType() == HintType.POST_FILTER && hint.getParams() != null && 0 < hint.getParams().length) {
                request.postFilter(QueryBuilders.wrapperQuery(hint.getParams()[0].toString()));
            }
        }
    }
    ...
    protected void updateRequestWithHighlight(Select select, SearchSourceBuilder request) {
    ...
    public abstract SearchResponse get(int s,int r)  throws SqlParseException,Exception;
}
