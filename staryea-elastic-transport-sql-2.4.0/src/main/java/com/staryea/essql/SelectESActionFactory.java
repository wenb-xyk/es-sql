...
public class SelectESActionFactory {
public static void main(String[] args) {
	SQLQueryExpr sqlExpr = (SQLQueryExpr) toSqlExpr("SELECT a as b FROM test ");
	ElasticLexer lexer = new ElasticLexer("SELECT a as b FROM test ");
	lexer.nextToken();
	MySqlStatementParser x = new MySqlStatementParser(lexer);
	...
	public static QueryAction create(Client client, String sql)
		...
		case "SELECT":
			...
			if (isJoin(sqlExpr, sql)) {
				JoinSelect joinSelect = new SqlParser()
						.parseJoinSelect(sqlExpr);
				handleSubQueries(client, joinSelect.getFirstTable());
				handleSubQueries(client, joinSelect.getSecondTable());
				return ESJoinQueryActionFactory.createJoinAction(client,
						joinSelect);
			} else {
		...
		default:
			throw new SQLFeatureNotSupportedException(String.format(
					"Unsupported query: %s", sql));
		}
	}

	private static void handleSubQueries(Client client, Select select)
	...
	private static void executeAndFillSubQuery(Client client,
			SubQueryExpression subQueryExpression, QueryAction queryAction)
			throws SqlParseException {
		List<Object> values = new ArrayList<>();
		Object queryResult;
		try {
			queryResult = QueryActionElasticExecutor.executeAnyAction(client,
					queryAction);
		} catch (Exception e) {
			throw new SqlParseException("could not execute SubQuery: "
					+ e.getMessage());
		}
	...
	private static boolean isJoin(SQLQueryExpr sqlExpr, String sql) {
		MySqlSelectQueryBlock query = (MySqlSelectQueryBlock) sqlExpr
